...
const BottomNav: React.FC<BottomNavProps> = ({ 
  ...
  const [nearestStops, setNearestStops] = useState<BusStop[]>([]);
  const [nearestBuses, setNearestBuses] = useState<BusPosition[]>([]);
  ...
  useEffect(() => {
    ...
    socket.on('busUpdate', (data: { buses: any[], busStops: any[] }) => {
  ...
  useEffect(() => {
    if (nearestStops.length === 0 && nearestBuses.length === 0 && userLocation) {
      // Demo data
      const demoStops = [
        { id: 1, name: 'Central Station', position: [28.6139, 77.2090] as [number, number], buses: ['1A', '2B', '3C'] },
        { id: 2, name: 'Town Hall', position: [28.6180, 77.2150] as [number, number], buses: ['2B', '4D', '6F'] },
        { id: 3, name: 'University', position: [28.6100, 77.2030] as [number, number], buses: ['1A', '5E', '7G'] },
      ];
      
      const demoBuses = [
        { id: '1A', route: '1A', position: [28.6150, 77.2095] as [number, number], nextStop: 'Central Station', eta: '2 min' },
        { id: '2B', route: '2B', position: [28.6170, 77.2140] as [number, number], nextStop: 'Town Hall', eta: '5 min' },
        { id: '3C', route: '3C', position: [28.6260, 77.2070] as [number, number], nextStop: 'Market Square', eta: '3 min' },
      ];
